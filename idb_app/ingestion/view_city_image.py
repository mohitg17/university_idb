...
# standalone script to quickly cycle through city images
...
query_set = CityImage.objects().limit(5)

fig, ax = plt.subplots()

time_delay_per_image = 0.1
for city_image in query_set:
    plt.cla()
    plt.imshow(Image.open(city_image.image))
    ax.set_title(f"{city_image.city.name}")
