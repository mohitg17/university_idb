...
for major in majors:
    ...
    if "Instructional content is defined" in definition or "Instructional content for this group of programs" in definition:
        cip_reference = get_cip_reference(definition)
        try:
            definition = cip_data.loc[cip_data['CIPCode'] == float(cip_reference)].iloc[0]['CIPDefinition']
        except ValueError:
            definition = "unavailable"
        if "Instructional content is defined" in definition or "Instructional content for this group of programs" in definition:
            try:
                definition = cip_data.loc[cip_data['CIPCode'] == float(cip_reference)].iloc[1]['CIPDefinition']
            except IndexError:
                pass
            if "Instructional content is defined" in definition or "Instructional content for this group of programs" in definition:
                definition = "unavailable"
