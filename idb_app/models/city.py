...
class City(Document, AbstractModel):
    ...
    def get_sort_buttons(cls) -> RadioButtonSet:
        return RadioButtonSet(
            title="Sort By",
            set_name="order_by",
            values=["median_gross_rent", "median_age", "population"],
    ...
    def get_template(self):
        ...
        schools = University.objects(school_city=self)[offset : offset + per_page]
        suggested_majors = (
            set(schools.first().majors_cip[:3])
            if (len(set(schools.first().majors_cip)) > 3)
            else set(schools.first().majors_cip)
        )
